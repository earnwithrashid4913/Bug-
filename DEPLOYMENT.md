...
GOATVERSE MD pairs a high-performance WhatsApp WebSocket engine with an integrated Web Pairing server (using `PORT`, default `3000`). Every supported deployment must use Node.js **20.9+**, one running replica, and persistent private storage for both authentication and runtime data.
...
5. The Web Pairing interface binds to `PORT` (default `3000`) for browser pairing and character theme selection where inbound HTTP is available. The included Render service is a Background Worker, so use its pairing code from logs instead.

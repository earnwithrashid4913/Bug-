...
3. Add the environment variables used for Render:
...
4. Set `BOT_NAME`, `THEME` or any other configuration values you want to override.
5. Deploy, generate a public domain (or use the Railway-provided port), and pair from the dashboard.
...
4. Set the environment variables (`OWNER_NAME`, `BOT_NUMBER`, `AUTH_DIR`, `DATA_DIR`) in Pterodactyl's startup/configuration panel.
...
6. Pair from the dashboard (allocate a port for the panel's Node.js server) or from the server console, and keep `AUTH_DIR` inside the server's persistent volume.
...
Set `AUTH_DIR` and `DATA_DIR` to paths that survive restarts and deployments, open the dashboard port in your firewall, and do not run a second copy of the bot against the same authentication directory.
...
A single Railway service with a persistent volume or a Render Web Service with a persistent disk is recommended. Both keep the long-running WhatsApp WebSocket alive, expose the pairing dashboard, and can retain the WhatsApp authentication state. A local machine/Termux installation is suitable for testing, but it must remain online for the bot to remain connected.

...
test('dashboard boots, paints the active theme and starts exactly two timers', async () => {
  ...
  assert.equal(dash.el.botName.textContent, 'GOATVERSE MD');
...
test('the UI only reports connected when the server says so', async () => {
  ...
  dash.setStatus({ state: 'connected', connected: true, message: 'GOATVERSE MD is connected to WhatsApp.', pairingCode: null });
